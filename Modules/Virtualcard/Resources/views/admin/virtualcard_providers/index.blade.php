...
@section('title', __('Provider Manager'))
    ...
    <div class="box box-default">
        <div class="box-body">
            <div class="d-flex justify-content-between">
                <div>
                    <div class="top-bar-title padding-bottom pull-left">{{ __('Provider Manager') }}</div>
                </div>
                <div>
                    @if(Common::has_permission(\Auth::guard('admin')->user()->id, 'add_card_provider') && isActive('ManualVirtualcard'))
                    <a href="{{ route('admin.virtualcard_provider.create') }}" class="btn btn-theme f-14"><span class="fa fa-plus"> &nbsp;</span>{{ __('Create Provider') }}</a>
                    @endif
                </div>
            </div>
        </div>
