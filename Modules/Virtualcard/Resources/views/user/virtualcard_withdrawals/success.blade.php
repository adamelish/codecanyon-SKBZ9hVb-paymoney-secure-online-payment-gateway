...
<div class="bg-white pxy-62 shadow">
    ...
    <p class="mb-0 text-center f-14 gilroy-medium text-gray-100 dark-CDO mt-6 r-mt-8 leading-25">
        {{ __('Your withdrawal request is currently under review. The admin will process it after the review is complete.') }}
    </p>
    ...
    <div class="amount-withdraw border-light-mode r-mt-24">
        ...
        <p class="mb-0 text-center gilroy-Semibold text-dark dark-A0 f-32 mt-8">{{ moneyFormat($virtualcardWithdrawalData['virtualcardWithdrawalData']->wallet?->currency?->code, formatNumber($virtualcardWithdrawalData['virtualcardWithdrawalData']->requestedFund, $virtualcardWithdrawalData['virtualcardWithdrawalData']->wallet?->currency?->id)) }}</p>
    </div>

    <div class="d-flex justify-content-center mt-28 r-mt-20">
        <a href="{{ route('user.virtualcard_withdrawal.pdf', $withdrawalData) }}" class="print-btn d-flex justify-content-center align-items-center gap-10" target="_blank">
            {!! svgIcons('printer') !!}
            <span>{{ __('Print') }}</span>
        </a>

        <a href="{{ route('user.virtualcard_withdrawal.create') }}" class="repeat-btn d-flex justify-content-center align-items-center ml-20">
