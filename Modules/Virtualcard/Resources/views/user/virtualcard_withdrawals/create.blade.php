...
@section('content')
    ...
    <div class="bg-white pxy-62" id="WithdrawalCreate">
        ...
        <form method="POST" action="{{ route('user.virtualcard_withdrawal.confirm') }}" id="withdrawalCreateForm">
            @csrf
            ...
            <div class="row">
                <div class="col-12">
                    <div class="mt-28 param-ref">
                        <label class="gilroy-medium text-gray-100 mb-2 f-15" for="virtualcardId">{{ __('Select Card') }}</label>
                        <div class="avoid-blink">
                            <select class="select2 sl_common_bx" name="virtualcardId" id="virtualcardId">
                                @foreach ($virtualcards as $virtualcard)
                                    <option value="{{ $virtualcard->id }}" data-icon="{{ $virtualcard->card_brand == 'Visa Card' ? virtualcardSvgIcons('visa_card_icon') : virtualcardSvgIcons('master_card_icon') }}"

                                        @if (old('virtualcardId') && old('virtualcardId') == $virtualcard->id) {{ 'selected' }}
                                        @elseif (!empty(session('paymentData')) && session('paymentData')['virtualcardId'] == $virtualcard->id) {{ 'selected' }}
                                        @endif
                                        >
                                        
                                        {{ maskCardNumberForLogo($virtualcard->card_number) }} ({{ cardTitle($virtualcard->virtualcardHolder) }})
                                    </option>
                                @endforeach
                            </select>
                        </div>
