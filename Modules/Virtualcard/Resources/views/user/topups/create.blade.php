...
@section('content')
<div class="bg-white pxy-62 shadow" id="topupCreate">
    ...
    <form method="post" action="{{ route('user.topup.confirm') }}" id="topupCreateForm">
        @csrf
        ...
        <div class="mt-28 param-ref">
            ...
            <div class="avoid-blink">
                <select class="select2" data-minimum-results-for-search="Infinity" name="virtualCardId" id="virtualCard">
                    @foreach ($virtualCards as $virtualCard)
                        <option data-type="{{ $virtualCard['type'] }}" value="{{ $virtualCard['id'] }}"  data-icon="{{ $virtualCard['cardicon'] }}"
                            @if (old('virtualCard') && old('virtualCard') == $virtualCard['id']) {{ 'selected="selected"' }}
                            @endif
                            >{{ $virtualCard['label'] }}</option>
                    @endforeach
                </select>
            </div>
        </div>
