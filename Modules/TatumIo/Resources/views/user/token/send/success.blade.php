...
<div class="bg-white pxy-62 shadow" id="crypto-send-success">
    ...
    <div class="d-flex justify-content-center mt-28 r-mt-20">
        <a href="{{ route('user.token_send_receive.print', $transactionId)}}" class="print-btn d-flex justify-content-center align-items-center gap-10" target="__blank">{!! svgIcons('printer') !!}<span>{{ __('Print') }}</span>
        </a>
        <a href="{{ route('tatumio.user.token_send.create', [encrypt($walletCurrencyCode), encrypt($walletId)]) }}" class="bg-white repeat-btn d-flex justify-content-center align-items-center ml-20">
