...
<div class="bg-white pxy-62 shadow" id="crypto-send-confirm">
    ...
    <p class="mb-0 text-center f-14 gilroy-medium text-gray dark-p mt-20">{{ __('Take a look before you send. Once the token sent to this address, its never be undone.') }}</p>

    <form action="{{ route('tatumio.user.token_send.success') }}" method="post" id="cryptoSendConfirmForm">
        @csrf
        <!-- Confirm Details -->
        <div class="mt-32 param-ref text-center">
            ...
            <p class="mb-0 gilroy-medium text-gray-100 sm-font-14">{{ __('Network Fee will be deducted from your native :x wallet', ['x' => $cryptoTrx['network']]) }}</p>
            ...
            <div class="mt-40 transaction-box">
                ...
                <div class="d-flex justify-content-between border-b-EF pb-13 mt-3">
                    ...
                    <p class="mb-0 f-16 gilroy-regular text-gray-100">{{ moneyFormat($cryptoTrx['network'], formatNumber($cryptoTrx['networkFee'], $currencyId)) }}</p>
                </div>
                ...
                <div class="d-flex justify-content-between mt-3 total">
                    ...
                    <p class="mb-0 f-16 gilroy-medium text-dark">{{ moneyFormat($cryptoTrx['currencySymbol'], formatNumber($cryptoTrx['amount'], $currencyId)) }}</p>
                </div>
            </div>
        </div>
