    ...
    <div class="row" id="virtualCardCategory">
        <div class="col-md-12">
            <div class="box box-info">
                ...
                <form method="post" action="{{ route('admin.card_categories.update', $cardCategory->id) }}" class="form-horizontal" id="virtualCardCategoryForm">
                    @csrf
                    @method('put')
                    <div class="box-body">
                        <!-- Name -->
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-6 col-md-6">
                                ...
                                >
                                    <option value='Active' {{ $cardCategory->status == 'Active' ? 'selected' : '' }}>{{ __('Active') }}</option>
                                    <option value='Inactive' {{ $cardCategory->status == 'Inactive' ? 'selected' : '' }}>{{ __('Inactive') }}</option>
                                </select>
                            </div>
                        </div>
                        <!-- Button -->
                        <div class="form-group row row">
                            <div class="col-sm-6 offset-md-3">
                                <a class="btn btn-theme-danger f-14 me-1" href="{{ route('admin.card_categories.index') }}" id="agent-package-cancel">{{ __('Cancel') }}</a>
                                @if(Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_card_category'))
                                <button type="submit" class="btn btn-theme f-14 float-right" id="virtualCardCategorySubmitBtn">
                                    <i class="fa fa-spinner fa-spin d-none"></i>
                                    <span id="virtualCardCategorySubmitBtnText">{{ __('Update') }}</span>
                                </button>
                                @endif
                            </div>
                        </div>
