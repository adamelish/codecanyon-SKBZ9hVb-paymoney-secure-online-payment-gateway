    ...
    <div class="box box-default">
        <div class="box-body">
            <div class="d-flex justify-content-between">
                <div>
                    <div class="top-bar-title padding-bottom pull-left">{{ __('Categories') }}</div>
                </div>
                <div>
                    @if(Common::has_permission(\Auth::guard('admin')->user()->id, 'add_card_category'))
                    <a href="{{ route('admin.card_categories.create') }}" class="btn btn-theme f-14"><span class="fa fa-plus"> &nbsp;</span>{{ __('Add Category') }}</a>
                    @endif
                </div>
            </div>
        </div>
