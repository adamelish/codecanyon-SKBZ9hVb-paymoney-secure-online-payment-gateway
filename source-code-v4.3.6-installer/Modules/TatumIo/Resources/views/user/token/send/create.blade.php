...
<div class="bg-white pxy-62 shadow" id="crypto-send-create">
    ...
    <form method="post" action="{{ route('tatumio.user.token_send.confirm') }}" id="crypto-send-form">
        ...
        <input name="network" type="hidden" value="{{ encrypt($network) }}"/>
        <input name="senderAddress" type="hidden" value="{{ $senderAddress }}"/>
        <input name="tokenAddress" type="hidden" value="{{ $cryptoToken->address }}"/>
        ...
        <div class="mt-20 label-top">
            ...
            <input type="text" class="form-control input-form-control apply-bg l-s2 receiverAddress" name="receiverAddress" id="receiverAddress" placeholder="{{ __('Enter valid recipient :x address', ['x' => $network]) }}" value="{{ session('transInfo')['receiverAddress'] ?? old('receiverAddress') }}" required data-value-missing="{{ __("Please provide a :x address.", ['x' => $walletCurrencyCode]) }}">
        ...
        <div class="mt-20 label-top">
            ...
            <p class="mb-0 text-gray-100 dark-B87 gilroy-regular f-12 mt-2">*{{ __('Only send :x token to this address, receiving any other coin will result in permanent loss.', ['x' => $walletCurrencyCode]) }}</p>
        </div>
        ...
        <div class="mt-20 label-top">
            <p class="mb-0 text-gray-100 dark-B87 gilroy-regular f-12 mt-2">*{{ __('The amount withdrawn/sent must at least be :x', ['x' => $minTatumIoLimit . ' ' .$walletCurrencyCode]) }}</p>
            <p class="mb-0 text-gray-100 dark-B87 gilroy-regular f-12 mt-2">*{{ __('Please keep at least :x for network fees', ['x' => 10 . ' ' . $network]) }}</p>
            <p class="mb-0 text-gray-100 dark-B87 gilroy-regular f-12 mt-2">*{{ __('Network Fee will be deducted from your native :x wallet', ['x' => $network]) }}</p>
        </div>
        <!-- End Amount -->
...
@push('js')
    ...
    <script type="text/javascript">
        ...
        var network = '{{ $network }}';
        var userId = '{{ auth()->id() }}';
        var walletId = '{{ encrypt($walletId) }}';
        var senderAddress = '{{ $senderAddress }}';
        var tokenAddress = '{{ $cryptoToken->address }}';
        var tokenDecimals = '{{ $cryptoToken->decimals }}';
        var validateAddressUrl = '{{ route("tatumio.user.token_send.validate_address") }}';
        var validateBalanceUrl = '{{ route("tatumio.user.token_send.validate_balance") }}';
        var cryptoSendConfirmUrl = '{{ route("tatumio.user.token_send.confirm") }}';
    ...
    <script src="{{ asset('Modules/TatumIo/Resources/assets/user/js/token_send_receive.min.js') }}" type="text/javascript"></script>
@endpush
