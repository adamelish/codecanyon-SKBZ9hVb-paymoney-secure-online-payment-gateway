...
if (!function_exists('getTatumIoTransactionReference')) {
    function getTatumIoTransactionReference($objectType, $network)
    {
        return (new CryptoAssetApiLog())
            ->where(['object_type' => $objectType, 'network' => $network])
            ->first()
            ->id;
    }
}
