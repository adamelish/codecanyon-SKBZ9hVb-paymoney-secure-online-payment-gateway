...
Route::get('initiate-cron', 'CronController@resetData');
...
//2fa
Route::group(['middleware' => ['guest:users', 'locale', 'check-user-inactive', 'verification'], 'namespace' => 'Users'], function () {
    Route::get('2fa', 'CustomerController@view2fa');
    Route::post('2fa-resend', 'CustomerController@resend2fa')->name('2fa-resend');
...
Route::group(['middleware' => ['guest:users', 'locale', 'twoFa', 'check-user-inactive', 'verification'], 'namespace' => 'Users'], function () {
    ...
    Route::group(['middleware' => ['permission:manage_setting']], function () {
        ...
        Route::post('prifile/update_password', 'CustomerController@updateProfilePassword')->name('user.profile.password.update')->middleware('demo_check');
        ...
        Route::post('prifile/update', 'CustomerController@updateProfileInfo')->name('user.profile.update')->middleware('demo_check');
...
Route::get('merchant/payment/success', 'ExpressMerchantPaymentController@paymentSuccess')->name('express.payment.succss');
Route::get('merchant/payment/redirect', 'ExpressMerchantPaymentController@redirectSuccessPath')->name('express.payment.redirect');
...
Route::match(array('GET', 'POST'), 'gateway/process-payment/{gateway}', 'GatewayController@processPayment')->name('gateway.process_payment');
